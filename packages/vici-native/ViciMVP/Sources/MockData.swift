...
struct TrainingPlan: Identifiable {
    ...
    static let mockCurrentPlan = TrainingPlan(
        id: "plan123",
        userId: "user123",
        status: .active,
    ...
    // Computed properties needed by views
    var durationInWeeks: Int {
        summary.durationWeeks
    }
    
    var goalDescription: String {
        if let raceName = goal.raceName, goal.type == .race {
            return "Race: \(raceName) (\(Int(goal.distance ?? 0)/1000)K)"
        } else {
    ...
    var currentWeekWorkouts: [Workout] {
        let now = Date()
        if let currentWeek = weeks.first(where: { $0.startDate <= now && $0.endDate >= now }) {
    ...
    var weeklyOverviews: [WeekSummary] {
        return weeks.map { week in
            return WeekSummary(
                ...
                focus: week.phase.rawValue,
                totalDistance: week.totalDistance / 1000, // km
                completedDistance: (week.completedDistance ?? 0) / 1000, // km
                progressPercentage: week.completionPercentage / 100 // 0-1 scale
            )
        }
    }
}
